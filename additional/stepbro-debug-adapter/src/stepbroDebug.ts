 ...
 *--------------------------------------------------------*/
/*
 * stepbroDebug.ts implements the Debug Adapter that "adapts" or translates the Debug Adapter Protocol (DAP) used by the client (e.g. VS Code)
 * into requests and events of the real "execution engine" or "debugger" (here: class StepBroRuntime).
 ...
 *
 * The most important class of the Debug Adapter is the StepBroDebugSession which implements many DAP requests by talking to the StepBroRuntime.
 */
...
import { StepBroRuntime, IRuntimeBreakpoint, FileAccessor, RuntimeVariable, timeout, IRuntimeVariableType } from './stepbroRuntime';
import { Subject } from 'await-notify';
import * as base64 from 'base64-js';

/**
 * This interface describes the stepbro-debug specific launch attributes
 * (which are not part of the Debug Adapter Protocol).
 * The schema for these attributes lives in the package.json of the stepbro-debug extension.
...
export class StepBroDebugSession extends LoggingDebugSession {

	// we don't support multiple threads, so we can use a hardcoded ID for the default thread
	private static threadID = 1;

	// a StepBro runtime (or debugger)
	private _runtime: StepBroRuntime;
	...
	public constructor(fileAccessor: FileAccessor) {
		super("stepbro-debug.txt");

		// this debugger uses zero-based lines and columns
		this.setDebuggerLinesStartAt1(false);
		this.setDebuggerColumnsStartAt1(false);

		this._runtime = new StepBroRuntime(fileAccessor);

        /**
		 * Setup event handlers
         * 
         * The StoppedEvent constructor can be called with 3 parameters
         * 
         * The first parameter "reason" is a string that gives the reason why we stop
         * This can be any reason, but the most common are the following:
         * 'step' | 'breakpoint' | 'exception' | 'pause' | 'entry' | 'goto'
         * 'function breakpoint' | 'data breakpoint' | 'instruction breakpoint'.
         * It is saved in the "reason" field on the body of the StoppedEvent 
         * defined in the Debug Adapter Protocol (DAP)
         * 
         * The optional second parameter "threadId" is a number which gives the threadID of the
         * stopped thread so VSCode knows which stack trace iit can look at. It is saved in the 
         * "threadId" field on the body of the StoppedEvent defined in the DAP
         * 
         * The optional third parameter is "exceptionText" which can be the description of
         * the exception occurring that created this stopped event. It is saved in the 
         * "text" field on the body of the StoppedEvent defined in the DAP
         */
		this._runtime.on('stopOnEntry', () => {
			this.sendEvent(new StoppedEvent('entry', StepBroDebugSession.threadID));
		});
		this._runtime.on('stopOnStep', () => {
			this.sendEvent(new StoppedEvent('step', StepBroDebugSession.threadID));
		});
		this._runtime.on('stopOnBreakpoint', () => {
			this.sendEvent(new StoppedEvent('breakpoint', StepBroDebugSession.threadID));
		});
		this._runtime.on('stopOnDataBreakpoint', () => {
			this.sendEvent(new StoppedEvent('data breakpoint', StepBroDebugSession.threadID));
		});
		this._runtime.on('stopOnInstructionBreakpoint', () => {
			this.sendEvent(new StoppedEvent('instruction breakpoint', StepBroDebugSession.threadID));
		});
		this._runtime.on('stopOnException', (exception) => {
			if (exception) {
				this.sendEvent(new StoppedEvent(`exception(${exception})`, StepBroDebugSession.threadID));
			} else {
				this.sendEvent(new StoppedEvent('exception', StepBroDebugSession.threadID));
			}
		});
	...
	protected threadsRequest(response: DebugProtocol.ThreadsResponse): void {
		...
		response.body = {
			threads: [
				new Thread(StepBroDebugSession.threadID, "thread 1"),
				new Thread(StepBroDebugSession.threadID + 1, "thread 2"),
			]
		};
		this.sendResponse(response);
	}
	...
	private createSource(filePath: string): Source {
		return new Source(basename(filePath), this.convertDebuggerPathToClient(filePath), undefined, undefined, 'stepbro-adapter-data');
	}
}

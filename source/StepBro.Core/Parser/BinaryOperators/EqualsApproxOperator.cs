...
{
    internal class EqualsApproxOperator : BinaryOperatorBase
    {
        public override SBExpressionData Resolve(StepBroListener listener, SBExpressionData first, SBExpressionData last)
        {
            if (first.IsConstant && last.IsConstant)
            {
                ...
                else if (f is double && s is double)
                {
                    return new SBExpressionData(
                        Execution.ExecutionHelperMethods.ApproximatelyEquals((double)f, (double)s));
                }
                else if (f is double && s is long)
                {
                    return new SBExpressionData(
                        Execution.ExecutionHelperMethods.ApproximatelyEquals((double)f, (long)s));
                }
                else if (f is long && s is double)
                {
                    return new SBExpressionData(
                        Execution.ExecutionHelperMethods.ApproximatelyEquals((long)f, (double)s));
                }
            ...
            else
            {
                ...
                if (first.IsValueType && last.IsValueType)
                {
                    ...
                    {
                        return new SBExpressionData(Expression.Equal(first.ExpressionCode, last.ExpressionCode));
                    }
                    else if (first.IsDecimal && last.IsDecimal)
                    {
                        return new SBExpressionData(
                            Expression.Call(
                                typeof(ExecutionHelperMethods).GetMethod("ApproximatelyEquals", new Type[] { typeof(double), typeof(double) }),
                    ...
                    else if (first.IsInt && last.IsDecimal)
                    {
                        return new SBExpressionData(
                            Expression.Call(
                                typeof(ExecutionHelperMethods).GetMethod("ApproximatelyEquals", new Type[] { typeof(long), typeof(double) }),
                                first.ExpressionCode,
                                last.ExpressionCode));
                    }
                    else if (first.IsDecimal && last.IsInt)
                    {
                        return new SBExpressionData(
                            Expression.Call(
                                typeof(ExecutionHelperMethods).GetMethod("ApproximatelyEquals", new Type[] { typeof(double), typeof(long) }),
                                first.ExpressionCode,
                                last.ExpressionCode));
                    }
                    else if (first.IsTimespan && last.IsTimespan)
                    {
                        return new SBExpressionData(Expression.Equal(first.ExpressionCode, last.ConvertToDouble().ExpressionCode));
                    }

        ...
        public override SBExpressionData Resolve(StepBroListener listener, SBExpressionData first, SBExpressionData last)
        {
            ...
            else
            {
                ...
                if (first.IsValueType && last.IsValueType)
                {
                    ...
                    {
                        return new SBExpressionData(Expression.GreaterThanOrEqual(first.ConvertToDouble().ExpressionCode, last.ExpressionCode));
                    }
                    else if (first.IsDecimal && last.IsInt)
                    {
                        return new SBExpressionData(Expression.GreaterThanOrEqual(first.ExpressionCode, last.ConvertToDouble().ExpressionCode));
                    }
                    ...
                    {
                        return new SBExpressionData(Expression.GreaterThanOrEqual(first.ConvertToInt64().ExpressionCode, last.ConvertToInt64().ExpressionCode));
                    }
                    else
                    {
                        throw new NotImplementedException();

                ...
                if (first.IsValueType && last.IsValueType)
                {
                    ...
                    else if (first.IsDecimal && last.IsDecimal)
                    {
                        return new SBExpressionData(
                            Expression.Call(
                                typeof(ExecutionHelperMethods).GetMethod(nameof(ExecutionHelperMethods.LessThanOrApprox), new Type[] { typeof(double), typeof(double) }),
                    ...
                    else if (first.IsInt && last.IsDecimal)
                    {
                        return new SBExpressionData(
                            Expression.Call(
                                typeof(ExecutionHelperMethods).GetMethod(nameof(ExecutionHelperMethods.LessThanOrApprox), new Type[] { typeof(long), typeof(double) }),
                                first.ExpressionCode,
                                last.ExpressionCode));
                    }
                    else if (first.IsDecimal && last.IsInt)
                    {
                        return new SBExpressionData(
                            Expression.Call(
                                typeof(ExecutionHelperMethods).GetMethod(nameof(ExecutionHelperMethods.LessThanOrApprox), new Type[] { typeof(double), typeof(long) }),
                                first.ExpressionCode,
                                last.ExpressionCode));
                    }

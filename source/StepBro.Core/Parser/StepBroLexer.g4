...
channels { COMMENTS_CHANNEL, DOC_COMMENTS_CHANNEL, DIRECTIVE }
...
TimespanLiteral
    :	INT ('.' [0-9]+)? ('s' | 'ms' | 'us' | 'ns')
    |	'@' ('0'..'9')+ COLON '0'..'5' '0'..'9' (COLON '0'..'5' '0'..'9')? ( '.' [0-9]+ )?
    ;

DateTimeLiteral : '@' DateTimeYear MINUS DateTimeMonth MINUS DateTimeDayOfMonth (' ' TimeOfDay ('UTC' | 'Local')? )? ;

// Range Constants

RangeLiteral : '@' OPEN_BRACKET RangeSection (COMMA RangeSection)* CLOSE_BRACKET ;

// Numeric constants

BinaryBlockLiteral : '@' (HexDigit HexDigit)+ ;
...
REGULAR_STRING :                    RegularStringFragment ;
VERBATIUM_STRING :                  VerbatiumStringFragment ;
...
IDENTIFIER_LITERAL :                RegularIdentifierFragment ;
...
// */
//COMMENT
//    :   '/*' .*? '*/'    -> channel(COMMENTS_CHANNEL) // match anything between /* and */
//    ;
...
DIRECTIVE_ERROR:               'error' Whitespace+              -> channel(DIRECTIVE), mode(DIRECTIVE_TEXT);
DIRECTIVE_WARNING:             'warning' Whitespace+            -> channel(DIRECTIVE), mode(DIRECTIVE_TEXT);
...
DIRECTIVE_DEFAULT:             'default'                        -> channel(DIRECTIVE);
...
DATATABLE_CELL_CONTENT:         (RegularStringFragment | VerbatiumStringFragment | ~["|\r\n\u0085\u2028\u2029])+ ;
DATATABLE_END_LINE :            NewLine                                                                         -> mode(DEFAULT_MODE);
DATATABLE_END_LINE_COMMENT:     Whitespace* ('//' ~[\r\n\u0085\u2028\u2029]*)? NewLine                          -> mode(DEFAULT_MODE);
DATATABLE_CELL_DELIMITER :      '|' ;

//mode DATATABLE_AFTER_ROW_MODE;

//DATATABLE_NEXT_ROW_START : DatatableRowStart -> mode(DATATABLE_ROW_MODE), type(DATATABLE_ROW_START);
//DATATABLE_SINGLE_LINE_COMMENT :     '//' InputCharacter* NewLine?   -> channel(COMMENTS_CHANNEL);
//DATATABLE_END :                     Whitespace* NewLine             -> mode(DEFAULT_MODE);

// Fragments

fragment RegularStringFragment : '"'  (~["\\\r\n\u0085\u2028\u2029] | CommonCharacter)* '"' ;
fragment VerbatiumStringFragment : '@"' (~'"' | '""')* '"' ;

fragment RegularIdentifierFragment : '\'' IdentifierOrKeyword '\'' ;

fragment InputCharacter:       ~[\r\n\u0085\u2028\u2029];

fragment DateTimeYear : '0'..'9' '0'..'9' '0'..'9' '0'..'9' ;
fragment DateTimeMonth : ('0' '1'..'9') | ('1' '0'..'2') ;
fragment DateTimeDayOfMonth : ('0' '0'..'9') | ('1'..'2' '0'..'9') | ('3' '0'..'1') ;

fragment TimeOfDay : TimeOfDayHours COLON TimeOfDayMS (COLON TimeOfDayMS ( '.' [0-9]+)? )? ;
fragment TimeOfDayHours : ('0' '1'..'9') | ('1' '0'..'9') | ('2' '0'..'3') ;
fragment TimeOfDayMS : '0'..'5' '0'..'9' ;
...
fragment IntLit : ( HexLiteral | INTEGER_LITERAL | IntegerWithSIPrefixLiteral ) ;

fragment HexDigit : [0-9] | [A-F] | [a-f];

fragment TwoDigitDecimalLiteral : [0-9] [0-9] ;
...
fragment
UnicodeEscape
    :   BACK_SLASH 'u' HexDigit HexDigit HexDigit HexDigit
    |   BACK_SLASH 'x' HexDigit HexDigit HexDigit HexDigit
    ;

//fragment FileOrFolder : (Letter|JavaIDDigit) (Letter|JavaIDDigit|DOT)* ;

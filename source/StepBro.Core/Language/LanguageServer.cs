...
{
    internal class LanguageServer, IOnLanguageServerInitialize, 
    {
        private Task<OmniLanguageServer> m_server = null;

        public void Start()
        {
            m_server = OmniLanguageServer.From(
                options =>
                    options
                        //.WithInput()
                        //.WithOutput()
                        //.ConfigureLogging(x => x.AddSe x.AddLanguageProtocolLogging(). )
                        .WithServerInfo(this.GetServerInfo())

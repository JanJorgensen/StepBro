...
internal static class ExecutionHelperMethods
{
    ...
    public static TVariable GetHostVariable<TVariable>(IScriptCallContext context, string name)
    {
        var variable = ServiceManager.Global.Get<IHost>().ListObjectContainers().FirstOrDefault(c => String.Equals(c.FullName, name, StringComparison.InvariantCulture)).Cast<TVariable>();
        if (variable == null)
        {
            context.ReportError($"Host application variable '{name}' not found.");
        }
        return variable;
    }

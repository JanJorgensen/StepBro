...
namespace StepBroCoreTest.Parser
{
    ...
    public class TestExpDotIdentifier
    {
        ...
        {
            var result = ExpressionParser.Parse("DateTime.Now");
            Assert.IsTrue(result.IsPropertyReference);
            Assert.AreEqual(typeof(DateTime), result.DataType.Type);
            Assert.AreEqual(typeof(DateTime).GetProperty("Now"), result.Value);
            var value = Expression.Lambda<Func<DateTime>>(result.ExpressionCode).Compile()();
            Assert.AreEqual(DateTime.Now.Date, value.Date);
        }
